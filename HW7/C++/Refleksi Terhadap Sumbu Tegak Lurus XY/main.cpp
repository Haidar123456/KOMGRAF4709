...
int main() {
    ...
    std::array<int, 3> P2 = {6, 8, 1};

    // Refleksi titik P1 terhadap sumbu tegak lurus bidang XY
    std::array<int, 3> reflected_P1 = reflectOverMatrix(P1, reflectionMatrix);

    // Refleksi titik P2 terhadap sumbu tegak lurus bidang XY
    std::array<int, 3> reflected_P2 = reflectOverMatrix(P2, reflectionMatrix);

    // Menampilkan hasil refleksi
    std::cout << "Titik P1: (" << P1[0] << ", " << P1[1] << ")\n";
    std::cout << "Titik P1 setelah refleksi: (" << reflected_P1[0] << ", " << reflected_P1[1] << ")\n";
    std::cout << "Titik P2: (" << P2[0] << ", " << P2[1] << ")\n";
    std::cout << "Titik P2 setelah refleksi: (" << reflected_P2[0] << ", " << reflected_P2[1] << ")\n";
